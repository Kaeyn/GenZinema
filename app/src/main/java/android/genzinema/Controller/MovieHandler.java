...
public class MovieHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(sql);
        sql = "INSERT OR IGNORE INTO " + TABLE_NAME +" VALUES (1,'Overlord','5','2','1tazr67-wERTdQ9tCdnJBiK3-P_ZqWJU0','Albedo, Titan, Victim, Coctyus','Trí Tony','2019','Cốt truyện anime sẽ đưa khán giả đến năm 2138 trong tương lai, khi khoa học công nghệ phát triển vượt bậc và ngành game thực tế ảo đang nở rộ hơn bao giờ hết. Yggdrasil, một game online vô cùng phổ biến thời gian đó bỗng dưng bị đóng cửa đột ngột','overlord','1S9Fj7wPhvFktzE5Pk4XWJ6ClLFRaadBW')";
        db.execSQL(sql);
        ...
        db.execSQL(sql);
        sql = "INSERT OR IGNORE INTO " + TABLE_NAME +" VALUES (9,'Nghệ thuật săn quỷ và nấu mì','1','2','19W4pCHeLpP1COAhlQFAS09wGde2-_D7f',' Jo Byung-gyu, Yoo Jun-sang, Kim Se-jeong, Yeom Hye-ran','Yo Seon-dong','2020','Ban ngày là nhân viên tiệm mì, ban đêm là thợ săn quỷ, họ dùng những khả năng đặc biệt để truy lùng lũ ma quỷ độc ác chuyên tấn công con người.','sanquyvami','1mcvF1aLucjkfB06Usm2sh_SZjzrqNxQB')";
        db.execSQL(sql);
        sql = "INSERT OR IGNORE INTO " + TABLE_NAME +" VALUES (10,'The Witcher','1','1','1JBxlijogEPvKjbqf5S5u8xWMLN1DUqPj','Henry Cavill, Freya Allan, Eamon Farren, Anya Chalotra, Joey Batey','Mike Ostrowski','2016','The Witcher là một sê-ri phim truyền hình chính kịch kỳ ảo của Mỹ được sản xuất bởi Lauren Schmidt Hissrich.','thewitcher','1JBxlijogEPvKjbqf5S5u8xWMLN1DUqPj')";
        db.execSQL(sql);
        sql = "INSERT OR IGNORE INTO " + TABLE_NAME +" VALUES (11,'The Flash','1','1','17DqvniSoy6PT0VaGYeMDcsb51rIMwpw8',' Jo Byung-gyu, Yoo Jun-sang, Kim Se-jeong, Yeom Hye-ran','Yo Seon-dong','2020','The Flash is a 2023 American superhero film based on the DC character of the same name. Produced by Warner Bros. Pictures, DC Studios, Double Dream, and the Disco Factory, and distributed by Warner Bros., it is the 13th installment in the DC Extended Universe (DCEU).','the_flash','17DqvniSoy6PT0VaGYeMDcsb51rIMwpw8')";
        db.execSQL(sql);
        sql = "INSERT OR IGNORE INTO " + TABLE_NAME +" VALUES (12,'Vương triều xác sống','2','2','1tazr67-wERTdQ9tCdnJBiK3-P_ZqWJU0','Henry Cavill, Freya Allan, Eamon Farren, Anya Chalotra, Joey Batey','Mike Ostrowski','2016','The Witcher là một sê-ri phim truyền hình chính kịch kỳ ảo của Mỹ được sản xuất bởi Lauren Schmidt Hissrich.','kingdom','16pmiD1vUO1mpNWU85vTgdsZ_fYiu4Ks4')";
        db.execSQL(sql);
        sql = "INSERT OR IGNORE INTO " + TABLE_NAME +" VALUES (13,'Điệu cha-cha-cha làng biển','4','2','1uAJ_UfmUVGDLyN9q3r6ElP5_2ESiEd_U','Henry Cavill, Freya Allan, Eamon Farren, Anya Chalotra, Joey Batey','Mike Ostrowski','2016','Điệu cha-cha-cha làng biển (Tiếng Hàn: 갯마을 차차차; Romaja: Gaenmaeul Chachacha) là bộ phim truyền hình Hàn Quốc ra mắt năm 2021 với sự tham gia diễn xuất của dàn diễn viên gồm Shin Min-ah, Kim Seon-ho và Lee Sang-yi.','chachacha','1nejTzBNkuLJmsXcbHH_uxWnW5zAXusN0')";
        db.execSQL(sql);
